...
# Slightly favor image and table over text
PREFERRED_LABELS = ["image", "table", "text"]
...
def pick_samples_for_doc(rows: List[Dict[str, Any]], per_doc: int) -> List[Dict[str, Any]]:
    """Return up to per_doc unique SPSC candidates, slightly favoring image and table labels."""
    ...
    # Slight weighting: multiply images/tables to increase selection probability
    weighted = []
    for r in unique:
        label = str(r.get("label", "")).lower()
        if label == "image":
            weighted.extend([r] * 3)  # triple weight for image
        elif label == "table":
            weighted.extend([r] * 5)  # double weight for table
        else:
            weighted.append(r)
    ...
    seen_pairs = set()
    for r in weighted:
        page_idx = to_int(r.get("page_index") or r.get("page_id", [0])[0])
