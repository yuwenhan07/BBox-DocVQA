...
def pick_samples_for_doc(rows: List[Dict[str, Any]], per_doc: int, three_prob: float = 0.2) -> List[Dict[str, Any]]:
    """Return up to `per_doc` MPMC entries (2-3 pages per entry, 1 crop per page).

    For each item, randomly choose 2 or 3 distinct pages within the doc; from each page, pick 1 crop
    preferring labels in PREFERRED_LABELS. Pages used in one item are not reused within the same doc item,
    but may be used again for later items if available.

    The probability of selecting 3 pages (instead of 2) is controlled by the `three_prob` parameter.
    """
    ...
    # We'll sample pages for each item independently to maximize chance to hit per_doc
    tries = 0
    max_tries = len(pages_all) * 3 + per_doc * 2  # guard against pathological cases

    while len(mpmc_entries) < per_doc and tries < max_tries:
        tries += 1
        # Need at least 2 pages to form an item
        if len(pages_all) < 2:
            break
        # Choose 2 or 3 pages with a controllable probability for 3-page samples
        k = 3 if random.random() < three_prob else 2
        k = min(k, len(pages_all))
        if k < 2:
            break
        # Randomly choose k distinct pages for this item
        chosen_pages = random.sample(pages_all, k)

        # From each chosen page, pick 1 crop with preference
        ...
        for pg in chosen_pages:
            ...
            # All pages had at least one candidate
            try:
                mpmc_entries.append(transform_group_mpmc(chosen_pages, chosen_crops))
            except Exception:
                continue
        # loop continues until per_doc or max_tries
...
def main():
    ap = argparse.ArgumentParser(description="Sample MPMC items per doc from a JSONL file (2-3 crops per page, 7 entries per doc by default).")
    ...
    ap.add_argument("--three-prob", type=float, default=0.2, help="Probability of sampling 3 pages for an MPMC item (default: 0.2).")
