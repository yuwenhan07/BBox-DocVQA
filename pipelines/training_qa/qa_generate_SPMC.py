...
def resolve_image_paths(row: Dict[str, Any], root: str) -> List[str]:
    """
    Determine absolute image paths for the row. Prefer explicit subimage_paths.
    Falls back to {root}/{category}/{doc_id}/{page_index}_{crop_id}.png pattern.
    """
    paths: List[str] = []
...
def build_prompt_text(system_prompt: str, doc_id: Any, page_ids: Sequence[Any]) -> str:
    ...
    if isinstance(page_ids, (int, str)):
        ...
        pages = list(page_ids) if isinstance(page_ids, Sequence) else []
...
def extract_single_qa(json_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # As a last resort, scan for balanced braces
        if not payload:
            raw = json_text
...
def main():
    parser = argparse.ArgumentParser(description="Generate SPMC QA per item using local Qwen-VL with vLLM.")
    ...
    parser.add_argument("--top-p", type=float, default=1, help="Top-p nucleus sampling")
    ...
    with open(args.out, mode, encoding="utf-8") as out_f:
        ...
        for idx in progress_iter:
            ...
            pil_images = load_images_from_paths(image_paths)
            if not pil_images:
                failed_indices.append(idx)
                continue
            ...
            prompt_text = build_prompt_text(system_prompt, doc_id=doc_id, page_ids=page_ids)
            messages = build_conversation(pil_images, prompt_text)
            if not messages:
                failed_indices.append(idx)
                continue
