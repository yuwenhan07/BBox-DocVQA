...
def pick_samples_for_doc(rows: List[Dict[str, Any]], per_doc: int) -> List[Dict[str, Any]]:
    """Return up to `per_doc` SPMC entries (one page per entry, exactly 2 crops).

    For each eligible page we keep two unique crops, preferring labels in `PREFERRED_LABELS` before filling
    ...
    preferred_pages = [p for p in pages if page_has_preferred.get(p, False)]
    other_pages = [p for p in pages if not page_has_preferred.get(p, False)]
    random.shuffle(preferred_pages)
    random.shuffle(other_pages)
    pages = preferred_pages + other_pages
    ...
    for page in pages:
        ...
        # Randomize within buckets and prefer non-text among the remaining candidates
        random.shuffle(preferred)
        non_text = [r for r in others if str(r.get("label", "")).lower() != "text"]
        text_only = [r for r in others if str(r.get("label", "")).lower() == "text"]
        random.shuffle(non_text)
        random.shuffle(text_only)
        others = non_text + text_only
...
def main():
    ap = argparse.ArgumentParser(description="Sample SPMC items per doc from a JSONL file (one page per entry, 2 crops per entry).")
