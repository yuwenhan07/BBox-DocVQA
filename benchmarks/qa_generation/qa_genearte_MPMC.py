...
def build_user_content(system_prompt: str, images_b64: List[str]):
    """
    Build the multimodal content payload for the Responses API using ALL crops.
    """
    content = [
...
def call_openai_responses(client, model: str, system_prompt: str, images_b64: List[str]) -> str:
...
def call_openai_chat(client, model: str, system_prompt: str, images_b64: List[str]) -> str:
    user_content = [{"type": "text", "text": "Rely solely on the provided image crops."}]
    for img_b64 in images_b64:
        if isinstance(img_b64, str) and img_b64:
    ...
    resp = oai.chat.completions.create(  # type: ignore[attr-defined]
        model=model,
        messages=messages,
        temperature=0,
    )
...
def extract_single_qa(json_text: str) -> Dict[str, Any]:
    ...
    or legacy:
      {"qas":[{"q":"...","a":"..."}]}
    Returns a dict with keys q, a. Returns empty dict on failure.
    """
    try:
...
def main():
    ...
    parser.add_argument("--sys-prompt-file", default=None, help="Optional file with custom system prompt")
    parser.add_argument("--sleep", type=float, default=0.0, help="Sleep seconds between API calls")
    parser.add_argument("--limit", type=int, default=10, help="Optionally limit the number of records to process")
    ...
    # Prepare OpenAI client (Responses API if available)
    client = None
    if _HAS_RESPONSES:
        try:
            client = OpenAI()
    ...
    with open(args.out, mode, encoding="utf-8") as out_f:
        ...
        for idx in progress_iter:
            ...
            # --- Align images with flattened crops order ---
            crop_ids = row.get("crop_ids") or []
            flat_crops = []
            if isinstance(crop_ids, list):
                for grp in crop_ids:
                    if isinstance(grp, list):
                        flat_crops.extend([c for c in grp if isinstance(c, int)])
            if isinstance(images_b64, list):
                n_needed = len(flat_crops) if flat_crops else len(images_b64)
                if len(images_b64) < n_needed:
                    print(f"[WARN] Row {idx}: images_b64 has {len(images_b64)} items but {n_needed} crops implied; proceeding with available images.")
                images_b64 = images_b64[:n_needed]
            else:
                images_b64 = []
            ...
            def request_once() -> str:
                if client is not None:
                    try:
                        return call_openai_responses(client, args.model, system_prompt, images_b64)
                    except Exception as resp_err:
                        print(f"[WARN] Responses API failed at index {idx}; trying Chat Completions fallback. Error: {resp_err}")
                        try:
                            return call_openai_chat(None, args.model, system_prompt, images_b64)
                        except Exception as chat_err:
                            raise RuntimeError("Chat Completions fallback also failed") from chat_err
                return call_openai_chat(None, args.model, system_prompt, images_b64)
